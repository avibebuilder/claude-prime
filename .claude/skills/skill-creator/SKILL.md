...
Before writing, search for existing skills that solve similar problems. Read `references/available-skill-resources.md` for curated skill repositories. Skim existing skills' SKILL.md to check scope relevance — extract reusable patterns, skip what doesn't fit, note gaps to fill.
...
These are optional patterns — not every skill needs them. Reach for them when the situation calls for it.

**Gotchas section** — Consider adding a Gotchas section and growing it over time. Each time Claude trips on something, add a line. This is often the highest-signal content in a skill — failure patterns that can't be derived from documentation alone. Example of how a billing-lib skill's gotchas might evolve:
```
## Gotchas                              # Week 2: first gotcha
- Proration rounds DOWN, not to nearest cent.

## Gotchas                              # Month 3: grown from real failures
- Proration rounds DOWN, not to nearest cent.
...
**First-run setup** — Use a `!` shell line in SKILL.md to detect whether the skill has been configured. The `!` line runs as a shell command before Claude reads the prompt:
```
## Your config
! cat ${CLAUDE_SKILL_DIR}/<skill-name>/config.json 2>/dev/null || echo "NOT_CONFIGURED"

## Instructions
If the config above is NOT_CONFIGURED, ask the user:
- Which Slack channel?
- Paste a sample standup you liked
Then write the answers to ${CLAUDE_SKILL_DIR}/<skill-name>/config.json.
